    ...
    functionDeclarations: [
      {
        ...
        parameters: {
          ...
          properties: {
            speakerName: { type: import_genai.Type.STRING, description: "Name of the speaker (e.g. 'Tanmay' or 'Guest')." },
            isTanmay: { type: import_genai.Type.BOOLEAN, description: "True ONLY if verified via secret password." }
          },
      ...
      },
      {
        ...
        parameters: {
          type: import_genai.Type.OBJECT,
          properties: {
            durationSeconds: { type: import_genai.Type.NUMBER, description: "Duration of the timer or alarm in seconds (e.g. 300 for 5 minutes)." },
            label: { type: import_genai.Type.STRING, description: "Optional title or purpose of the timer e.g. 'boil eggs' or 'study session'." },
            isAlarm: { type: import_genai.Type.BOOLEAN, description: "Set to true if user asked for an alarm, false for countdown timer." }
          },
      ...
      },
      {
        ...
        parameters: {
          type: import_genai.Type.OBJECT,
          properties: {
            query: { type: import_genai.Type.STRING, description: "Optional filter topic e.g. 'time', 'weather', 'location', or 'general'." }
          }
        }
      },
      {
        ...
        parameters: {
          type: import_genai.Type.OBJECT,
          properties: {
            url: { type: import_genai.Type.STRING, description: "Direct URL or platform destination e.g. https://youtube.com" },
            query: { type: import_genai.Type.STRING, description: "Optional search query or video title e.g. 'anime music' or 'SAO theme'" },
            target: { type: import_genai.Type.STRING, description: "Destination e.g. 'youtube', 'google', 'github', 'wikipedia' or 'custom'" }
          },
          required: ["url"]
        }
      },
      {
        ...
        parameters: {
          type: import_genai.Type.OBJECT,
          properties: {
            query: { type: import_genai.Type.STRING, description: "Search query string." },
            openInBrowser: { type: import_genai.Type.BOOLEAN, description: "Set to true ONLY if Tanmay explicitly instructed to open a visible Google search window (e.g. 'Google par search karke open karo'). Defaults to false for silent search." }
          },
          required: ["query"]
        }
      },
      {
        ...
        parameters: {
          type: import_genai.Type.OBJECT,
          properties: {
            memoryText: { type: import_genai.Type.STRING, description: "The fact or instruction user explicitly asked to save." },
            category: { type: import_genai.Type.STRING, description: "Category like 'user_command', 'preference', 'personal', or 'schedule'." }
          },
          required: ["memoryText"]
        }
      },
      {
        ...
        parameters: {
          type: import_genai.Type.OBJECT,
          properties: {
            memoryQuery: { type: import_genai.Type.STRING, description: "Text or keyword of the memory to erase." }
          },
      ...
      },
      {
        ...
        parameters: {
          type: import_genai.Type.OBJECT,
          properties: {
            requestType: { type: import_genai.Type.STRING, description: "'current_location', 'nearby', or 'directions'" },
            query: { type: import_genai.Type.STRING, description: "Category or search term e.g. 'cafe', 'hospital', or destination address." },
            destination: { type: import_genai.Type.STRING, description: "Destination place name or address for turn-by-turn navigation." }
          },
      ...
      },
      {
        ...
        parameters: {
          type: import_genai.Type.OBJECT,
          properties: {
            show: { type: import_genai.Type.BOOLEAN, description: "Set to true to open map view overlay on UI." },
            title: { type: import_genai.Type.STRING, description: "Map overlay title e.g. 'Current Location' or 'Route to Connaught Place'." },
            query: { type: import_genai.Type.STRING, description: "Location or destination query for map marker." }
          },
          required: ["show"]
        }
      },
      {
        ...
        parameters: {
          type: import_genai.Type.OBJECT,
          properties: {
            query: { type: import_genai.Type.STRING, description: "Song, artist, or video search query (e.g., 'Arijit Singh naya gana', 'Kesariya song', 'Lofi hip hop')" }
          },
          required: ["query"]
        }
      },
      ...
      },
      {
        ...
        parameters: {
          type: import_genai.Type.OBJECT,
          properties: {
            show: { type: import_genai.Type.BOOLEAN, description: "Set to true to show Developer Dashboard, false to hide." },
            activeTab: { type: import_genai.Type.STRING, description: "Tab to view: 'analysis', 'check_code', or 'code_stream'." }
          },
          required: ["show"]
        }
      },
      ...
      },
      {
        ...
        parameters: {
          type: import_genai.Type.OBJECT,
          properties: {
            targetFile: { type: import_genai.Type.STRING, description: "Optional specific file to audit." }
          }
        }
      },
      {
        ...
        parameters: {
          type: import_genai.Type.OBJECT,
          properties: {
            issueDescription: { type: import_genai.Type.STRING, description: "Description of the bugs to fix." },
            filePath: { type: import_genai.Type.STRING, description: "Target file path e.g. 'server.ts' or 'App.tsx'." }
          },
      ...
      },
      {
        ...
        parameters: {
          type: import_genai.Type.OBJECT,
          properties: {
            filePath: { type: import_genai.Type.STRING, description: "File path in repository to update." },
            code: { type: import_genai.Type.STRING, description: "Verified code content." },
            commitMessage: { type: import_genai.Type.STRING, description: "Commit message." }
          },
          required: ["filePath"]
        }
      }
...
app.post("/api/check-code", async (req, res) => {
  try {
    ...
    let realContent = "";
    const primaryPath = import_path.default.join(process.cwd(), target);
    const secondaryPath = import_path.default.join(process.cwd(), "src", target);
    if (import_fs.default.existsSync(primaryPath)) {
      realContent = import_fs.default.readFileSync(primaryPath, "utf-8");
    } else if (import_fs.default.existsSync(secondaryPath)) {
      realContent = import_fs.default.readFileSync(secondaryPath, "utf-8");
    }
...
${realContent.slice(0, 1e5)}`;
    ...
    res.json({
      ...
      promptQuestion: issuesFound && issuesFound.length > 0 ? "Tanmay Bhaiya, real codebase me ye issues mile hain. Kya main inko fix kar doon?" : "Tanmay Bhaiya, real codebase bilkul clean hai! Kya aap koi optimization try karna chahte hain?"
    });
  } catch (err) {
...
app.post("/api/refine-code", async (req, res) => {
  try {
    ...
    if (!baseCode) {
      const primaryPath = import_path.default.join(process.cwd(), target);
      if (import_fs.default.existsSync(primaryPath)) {
        baseCode = import_fs.default.readFileSync(primaryPath, "utf-8");
      }
    }
    if (!baseCode) {
    ...
    if (apiKeyFlash && apiKeyFlash !== "dummy_key_for_startup") {
      try {
        const aiFlash = new import_genai.GoogleGenAI({ apiKey: apiKeyFlash });
...
${baseCode.slice(0, 1e5)}`;
    ...
    if (apiKeyPro && apiKeyPro !== "dummy_key_for_startup") {
      try {
        const aiPro = new import_genai.GoogleGenAI({ apiKey: apiKeyPro });
...
app.post("/api/github-deploy", async (req, res) => {
  ...
  try {
    ...
    if (updateRes.ok || updateRes.status === 201) {
      try {
        const localPath = import_path.default.join(process.cwd(), target);
        if (import_fs.default.existsSync(localPath)) {
          import_fs.default.writeFileSync(localPath, code, "utf-8");
        }
      } catch (e) {
      ...
      session = await ai.live.connect({
        ...
        callbacks: {
          onmessage: (message) => {
            ...
            const toolCall = message.toolCall || message.serverContent?.modelTurn?.parts?.[0]?.functionCall;
            if (toolCall) {
